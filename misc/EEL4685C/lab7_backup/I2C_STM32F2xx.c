/*-----------------------------------------------------------------------------
 * Name:    I2C_STM32F2xx.c
 * Purpose: I2C low level routines for the STM32F2xx
 * Note(s):
 *-----------------------------------------------------------------------------
...
void io_pin (uint32_t io, uint32_t st) {
  if (io & IO_SDA) {
    if (st == IO_LOW) GPIOB->BSRR |= (1 << (9+16));  // no BSRR in new GPIO_typedef, so manually shift by another 16 - FOSTER
    else              GPIOB->BSRR |= (1 << 9);
  }
  if (io & IO_SCL) {
    if (st == IO_LOW) GPIOB->BSRR |= (1 << (8+16));
    else              GPIOB->BSRR |= (1 << 8);
  }
}
